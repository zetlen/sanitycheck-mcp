...
export function resolveService(query: string): ServiceEntry[] {
  ...
  const substring = SERVICES.filter((s) => s.name.toLowerCase().includes(q) || s.slug.includes(q));
  return substring;
}
