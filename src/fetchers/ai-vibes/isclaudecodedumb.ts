...
export async function fetchIsClaudeCodeDumb(model: string): Promise<VibeResult | null> {
  ...
  if (!model.toLowerCase().includes("claude")) {
    return {
      ...
      sentiment: "N/A (Claude-only tracker)",
  ...
  try {
    ...
    const relevant = lines
      .filter((l) => /dumb|smart|yes|no|vote|percent|%|\d+/i.test(l))
      .slice(0, 5);

    const sentiment =
      relevant.length > 0 ? relevant.join("; ").slice(0, 200) : "Could not parse vote results";
